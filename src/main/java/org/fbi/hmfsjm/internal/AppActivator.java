...
public class AppActivator implements BundleActivator {
    ...
    public void start(BundleContext context) {
        ...
        properties.put("APPID", "HMFSJM");
        context.registerService(ProcessorManagerService.class.getName(), factory, properties);

        logger.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + " - Starting the HMFS-JM app bundle...." );
    }

    public void stop(BundleContext context) throws Exception {
        AppActivator.context = null;
        this.serverService.stop();
        logger.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + " - Stopping the HMFS-JM app bundle...");
    }
